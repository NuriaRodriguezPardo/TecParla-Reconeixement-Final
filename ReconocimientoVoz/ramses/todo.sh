...
if [ $FUNK_PRM == periodograma ]; then
    ...
    [ -d $(dirname $EXEC_PREV) ] || mkdir -p $(dirname $EXEC_PREV)
    echo "##########################################################" | tee $EXEC_PREV
    echo "# Uso del estimador de Máxima Entropía #" | tee -a $EXEC_PREV
    echo "##########################################################" | tee -a $EXEC_PREV
    echo "exec(open('$DIR_WRK/ramses/maxima_entropia.py').read())" | tee -a $EXEC_PREV
    echo "import numpy as np" | tee -a $EXEC_PREV
